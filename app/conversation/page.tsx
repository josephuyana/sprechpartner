...
export default function Conversation() {
  ...
  const handleRestart = useCallback(() => {
    resetAndStart();
    startConversation();
  }, [resetAndStart, startConversation]);
  ...
  const transcribeAndRespond = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        newTurns.push({ user: transcript, assistant: responseText });
        setTurns(newTurns);
        setIsResponding(false);
  ...
  const isLoading = isTranscribing || isResponding || isStarting;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-zinc-100 px-4 py-8">
      ...
      >
        <span
          className={`w-4 h-4 rounded-full ${
            isRecording ? "bg-red-500 animate-pulse" : "bg-red-500"
          }`}
        />
      </button>
      ...
      {isGettingFeedback && (
        <p className="text-sm text-zinc-500">Generating feedback…</p>
      )}
          ...
          <div key={i} className="flex flex-col gap-2">
            ...
            {turn.assistant ? (
              <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700">
                <p className="text-xs font-medium text-zinc-400 mb-1">Teacher</p>
                {turn.assistant}
              </div>
            ) : null}
          </div>
